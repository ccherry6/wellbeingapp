...
CREATE TABLE public.auto_alert_logs (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_wellness_entries_date ON public.wellness_entries(entry_date);
CREATE INDEX idx_login_sessions_cleanup ON public.login_sessions(expires_at, is_used);
...
CREATE INDEX idx_wellness_activities_date ON public.wellness_activities(completed_at);
CREATE INDEX idx_wellness_resources_metric ON public.wellness_resources(metric_key);
CREATE INDEX idx_wellness_resources_condition ON public.wellness_resources(trigger_condition, trigger_value);
CREATE INDEX idx_wellness_resources_type ON public.wellness_resources(resource_type);
CREATE INDEX idx_coach_alerts_coach_id ON public.coach_alerts(coach_id);
CREATE INDEX idx_coach_alerts_student_id ON public.coach_alerts(student_id);
CREATE INDEX idx_coach_alerts_metric ON public.coach_alerts(metric);
CREATE INDEX idx_coach_alerts_active ON public.coach_alerts(is_active);
CREATE INDEX idx_coach_alerts_triggered ON public.coach_alerts(last_triggered_at);
CREATE INDEX idx_auto_alert_logs_student_id ON public.auto_alert_logs(student_id);
CREATE INDEX idx_auto_alert_logs_metric ON public.auto_alert_logs(metric_triggered);
CREATE INDEX idx_auto_alert_logs_date ON public.auto_alert_logs(alert_sent_at);
...
-- Note: The handle_new_user trigger would typically be created on auth.users table
-- but that requires superuser privileges. Instead, the application handles user profile creation.